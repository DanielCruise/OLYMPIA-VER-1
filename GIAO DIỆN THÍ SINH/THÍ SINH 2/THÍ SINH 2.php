...
<div id="login">
    <form>
        <label>THÍ SINH 2: </label> <input type="text" id="name2">
    </form>
...
<script>
    ...
    function sendName() {
        var name2 = $('#name2').val();
        document.getElementById("nameOfCan").innerHTML = "THÍ SINH 2: " + name2.toUpperCase();
        $.ajax({
            url: "FILE XÁC NHẬN/XÁC NHẬN TÊN/XÁC NHẬN 2.php",
            type: "POST",
            data: {'name2': name2.toUpperCase()},
            success: function () {
    ...
    function sendObsAns() {
        var obsAns2 = $('#sentObs').val();
        if (timePane.innerHTML >= 0)
            $.ajax({
                url: "FILE XÁC NHẬN/TRẢ LỜI VCNV/CNV2.php",
                type: "POST",
                data: {'obsAns2': obsAns2.toUpperCase()},
                success: function () {
    ...
    function sendAcceAns() {
        var acceAns2 = $('#sentAcce').val();
        if (timePane.innerHTML >= 0)
            $.ajax({
                url: "FILE XÁC NHẬN/TRẢ LỜI TĂNG TỐC/TLTT2.php",
                type: "POST",
                data: {'acceAns2': acceAns2.toUpperCase(), 'time2': acceTimeAns / 100},
                success: function () {
    ...
    function grantedObsAns() {
        $.ajax({
            url: "FILE XÁC NHẬN/TRẢ LỜI CNV/TLCNV2.php",
            type: "POST",
            data: {'grantedAns2': 1},
            success: function () {
            }
    ...
    function finishingGranted() {
        if (allowed.innerHTML == '1') {
            $.ajax({
                url: "FILE XÁC NHẬN/CƯỚP ĐIỂM VỀ ĐÍCH/FIGranted2.php",
                type: "POST",
                data: {'grantedFinishing2': 1},
                success: function () {
                }
            });
        } else {
    ...
    function warmingUpConnect() {
        ...
        if (document.getElementById('turnWarmingUp').innerHTML == '2') {
