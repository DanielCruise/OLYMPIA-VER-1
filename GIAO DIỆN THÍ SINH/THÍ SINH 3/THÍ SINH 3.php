...
<div id="login">
    <form>
        <label>THÍ SINH 3: </label> <input type="text" id="name3">
    </form>
...
<script>
    ...
    function sendName() {
        var name3 = $('#name3').val();
        document.getElementById("nameOfCan").innerHTML = "THÍ SINH 3: " + name3.toUpperCase();
        $.ajax({
            url: "FILE XÁC NHẬN/XÁC NHẬN TÊN/XÁC NHẬN 3.php",
            type: "POST",
            data: {'name3': name3.toUpperCase()},
            success: function () {
    ...
    function sendObsAns() {
        var obsAns3 = $('#sentObs').val();
        if (timePane.innerHTML >= 0)
            $.ajax({
                url: "FILE XÁC NHẬN/TRẢ LỜI VCNV/CNV3.php",
                type: "POST",
                data: {'obsAns3': obsAns3.toUpperCase()},
                success: function () {
    ...
    function sendAcceAns() {
        var acceAns3 = $('#sentAcce').val();
        if (timePane.innerHTML >= 0)
            $.ajax({
                url: "FILE XÁC NHẬN/TRẢ LỜI TĂNG TỐC/TLTT3.php",
                type: "POST",
                data: {'acceAns3': acceAns3.toUpperCase(), 'time3': acceTimeAns / 100},
                success: function () {
    ...
    function grantedObsAns() {
        $.ajax({
            url: "FILE XÁC NHẬN/TRẢ LỜI CNV/TLCNV3.php",
            type: "POST",
            data: {'grantedAns3': 1},
            success: function () {
            }
    ...
    function finishingGranted() {
        if (allowed.innerHTML == '1') {
            $.ajax({
                url: "FILE XÁC NHẬN/CƯỚP ĐIỂM VỀ ĐÍCH/FIGranted3.php",
                type: "POST",
                data: {'grantedFinishing3': 1},
                success: function () {
                }
            });
        } else {
    ...
    function warmingUpConnect() {
        ...
        if (document.getElementById('turnWarmingUp').innerHTML == '3') {
