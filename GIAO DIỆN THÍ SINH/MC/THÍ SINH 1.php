...
<div id="login">
    <form>
        <label>THÍ SINH 1: </label> <input type="text" id="name1">
    </form>
...
<script>
    ...
    function sendName() {
        var name1 = $('#name1').val();
        document.getElementById("nameOfCan").innerHTML = "THÍ SINH 1: " + name1.toUpperCase();
        $.ajax({
            url: "FILE XÁC NHẬN/XÁC NHẬN TÊN/XÁC NHẬN 1.php",
            type: "POST",
            data: {'name1': name1.toUpperCase()},
            success: function () {
    ...
    function sendAcceAns() {
        var acceAns1 = $('#sentAcce').val();
        if (timePane.innerHTML >= 0)
            $.ajax({
                url: "FILE XÁC NHẬN/TRẢ LỜI TĂNG TỐC/TLTT1.php",
                type: "POST",
                data: {'acceAns1': acceAns1.toUpperCase(), 'time1': acceTimeAns / 100},
                success: function () {
    ...
    function finishingGranted() {
        if (allowed.innerHTML == '1') {
            $.ajax({
                url: "FILE XÁC NHẬN/CƯỚP ĐIỂM VỀ ĐÍCH/FIGranted1.php",
                type: "POST",
                data: {'grantedFinishing1': 1},
                success: function () {
                }
            });
        } else {
    ...
    function warmingUpConnect() {
        ...
        if (document.getElementById('turnWarmingUp').innerHTML == '1') {
